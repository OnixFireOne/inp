...
export default function LinkCategoriesPage() {
  ...
  // Pagination disabled to show all categories (no hidden limits).
                      ...
                      {rows.map((r) => (
                        <li key={r.key} className="px-3 py-2 text-sm flex items-center gap-3">
                          ...
                          <button
                            ...
                            className="text-[var(--text-mut)] hover:text-[var(--text)] cursor-pointer mr-2 btn-ghost"
                          ...
                          <button
                            ...
                            className="text-xs text-rose-600 hover:text-rose-500 cursor-pointer btn-ghost"
                            aria-label="Удалить"
                          >
                            ×
                          </button>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-[var(--surface)] rounded-xl w-full max-w-sm p-5 space-y-3">
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button onClick={() => setEditing(null)} className="px-3 py-1.5 rounded border text-sm btn-default">
                Отмена
              </button>
            </div>
          </div>
...
function CategoryRow({
  ...
  return (
    <li ref={setNodeRef} style={style} className="flex items-center gap-3 px-3 py-2 text-sm bg-[var(--surface)]">
      ...
      <button onClick={onEdit} className="text-[var(--text-mut)] hover:text-[var(--text)] cursor-pointer mr-2 btn-ghost" aria-label="Редактировать">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
          <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
        </svg>
